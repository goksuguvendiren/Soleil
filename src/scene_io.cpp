...
static void write_cameraA(CameraIO* camera, FILE* fp)
{
    ...
    fprintf(fp, "  orthoUp %g %g %g\n", camera->orthoUp[0], camera->orthoUp[1], camera->orthoUp[2]);
...
static void read_cameraA(SceneIO* scene, FILE* fp)
{
    ...
    fscanf(fp, " {");
    CHECK(3, fscanf(fp, " position %g %g %g", &camera->position[0], &camera->position[1], &camera->position[2]));
    CHECK(3, fscanf(fp, " viewDirection %g %g %g", &camera->viewDirection[0], &camera->viewDirection[1],
                    &camera->viewDirection[2]));
    CHECK(1, fscanf(fp, " focalDistance %g", &camera->focalDistance));
    CHECK(3, fscanf(fp, " orthoUp %g %g %g", &camera->orthoUp[0], &camera->orthoUp[1], &camera->orthoUp[2]));
...
static void read_point_lightA(SceneIO* scene, FILE* fp)
{
    ...
    fscanf(fp, " {");
    CHECK(3, fscanf(fp, " position %g %g %g", &light->position[0], &light->position[1], &light->position[2]));
    CHECK(3, fscanf(fp, " color %g %g %g", &light->color[0], &light->color[1], &light->color[2]));
    fscanf(fp, " }");
}
...
static void read_directional_lightA(SceneIO* scene, FILE* fp)
{
    ...
    fscanf(fp, " {");
    CHECK(3, fscanf(fp, " direction %g %g %g", &light->direction[0], &light->direction[1], &light->direction[2]));
    CHECK(3, fscanf(fp, " color %g %g %g", &light->color[0], &light->color[1], &light->color[2]));
    fscanf(fp, " }");
}
...
MaterialIO* new_material(long n)
{
...
{
    fprintf(fp, "  material {\n");
    fprintf(fp, "    diffColor %g %g %g\n", material->diffColor[0], material->diffColor[1], material->diffColor[2]);
    fprintf(fp, "    ambColor %g %g %g\n", material->ambColor[0], material->ambColor[1], material->ambColor[2]);
    fprintf(fp, "    specColor %g %g %g\n", material->specColor[0], material->specColor[1], material->specColor[2]);
    fprintf(fp, "    emisColor %g %g %g\n", material->emissColor[0], material->emissColor[1], material->emissColor[2]);
...
static void read_materialA(MaterialIO* material, FILE* fp)
{
    fscanf(fp, " material {");
    CHECK(3,
          fscanf(fp, " diffColor %g %g %g", &material->diffColor[0], &material->diffColor[1], &material->diffColor[2]));
    CHECK(3, fscanf(fp, " ambColor %g %g %g", &material->ambColor[0], &material->ambColor[1], &material->ambColor[2]));
    CHECK(3,
          fscanf(fp, " specColor %g %g %g", &material->specColor[0], &material->specColor[1], &material->specColor[2]));
    CHECK(3, fscanf(fp, " emisColor %g %g %g", &material->emissColor[0], &material->emissColor[1],
                    &material->emissColor[2]));
...
static void write_sphereA(ObjIO* obj, FILE* fp)
{
    ...
    fprintf(fp, "  radius %g\n", sphere->radius);
    fprintf(fp, "  xaxis %g %g %g\n", sphere->xaxis[0], sphere->xaxis[1], sphere->xaxis[2]);
    fprintf(fp, "  xlength %g\n", sphere->xlength);
    fprintf(fp, "  yaxis %g %g %g\n", sphere->yaxis[0], sphere->yaxis[1], sphere->yaxis[2]);
    fprintf(fp, "  ylength %g\n", sphere->ylength);
    fprintf(fp, "  zaxis %g %g %g\n", sphere->zaxis[0], sphere->zaxis[1], sphere->zaxis[2]);
...
static void read_sphereA(SceneIO* scene, FILE* fp)
{
    ...
    fscanf(fp, " {");
    read_objectA(obj, fp);
    fscanf(fp, " origin %g %g %g", &sphere->origin[0], &sphere->origin[1], &sphere->origin[2]);
    fscanf(fp, " radius %g", &sphere->radius);
    fscanf(fp, " xaxis %g %g %g", &sphere->xaxis[0], &sphere->xaxis[1], &sphere->xaxis[2]);
    fscanf(fp, " xlength %g", &sphere->xlength);
    fscanf(fp, " yaxis %g %g %g", &sphere->yaxis[0], &sphere->yaxis[1], &sphere->yaxis[2]);
    fscanf(fp, " ylength %g", &sphere->ylength);
    fscanf(fp, " zaxis %g %g %g", &sphere->zaxis[0], &sphere->zaxis[1], &sphere->zaxis[2]);
...
static void delete_sphere(SphereIO* sphere)
{
    free(sphere);
}
...
static void read_poly_setA(SceneIO* scene, FILE* fp)
{
    ...
    for (i = 0; i < pset->numPolys; i++, poly++)
    {
        ...
        for (j = 0; j < poly->numVertices; j++, vert++)
        {
            CHECK(3, fscanf(fp, " pos %g %g %g", &vert->pos[0], &vert->pos[1], &vert->pos[2]));
            if (pset->normType == PER_VERTEX_NORMAL)
            {
                CHECK(3, fscanf(fp, " norm %g %g %g", &vert->norm[0], &vert->norm[1], &vert->norm[2]));
            }
            if (pset->materialBinding == PER_VERTEX_MATERIAL)
            {
