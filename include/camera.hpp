...
public:
    camera() = default;
    camera(const glm::vec3 &pos, const glm::vec3 &view, const glm::vec3 &up, float foc, float fov, float ipd = 1.0f, bool pin = true) : eye_pos(pos), view_dir(glm::normalize(view)), up_dir(glm::normalize(up)), focal_dist(foc), vert_fov(fov), pinhole(pin), image_plane_dist(ipd)
    {
        if (pin)
        {
